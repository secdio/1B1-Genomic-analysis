#参考自https://github.com/abacus-gene/paml-tutorial/blob/main/positive-selection/01_protocol_analyses/01_site_models/Find_bestmodel.R
#-------------------#
# CLEAN ENVIRONMENT #
#-------------------#
rm( list = ls( ) )
...
lnL_vals <- read.table( file ="C:/Users/Admin/Desktop/PAML/site_models_codeml/lnL_sites.txt", sep= " ", stringsAsFactors = FALSE, 
                        header = TRUE )
...
text( x = coords_dev[1], y = coords_dev[2],
      labels = expression( atop( paste( '2', Delta, 'l = 311.9741', sep = "" ) ) ),
      cex = 1.2, col = "red" )
...
text( x = coords_pval[1], y = coords_pval[2],
      labels = expression( atop( ''*italic( pval )*' = 8.112994e-70' ) ),
      cex = 1.2, col = "black" )
title( "A) M0 vs M1a" )
...
text( x = coords_dev[1], y = coords_dev[2],
      labels = expression( atop( paste( '2', Delta, 'l = 4.213732', sep = "" ) ) ),
      cex = 1.2, col = "red" )
text( x = coords_alphac[1], y = coords_alphac[2],
      labels = expression( atop( paste( chi["2,0.05"]^"2", "= 5.99", sep = " " ) ) ),
      cex = 1.2, col = "gray" )
text( x = coords_alphac2[1], y = coords_alphac2[2],
      ...
      cex = 1.2, col = "brown" )
text( x = coords_pval[1], y = coords_pval[2],
      labels = expression( atop( ''*italic( pval )*' = 0.1216185' ) ),
      cex = 1.2, col = "black" )
title( "C) M7 vs M8" )
